...
/**
 * @enum SpectrumOrTime
 * @brief S Bit (SpectrumOrTime) field in the VRT Context Packet Header specific bits
 * 
 * Constants for the 1-bit S Bit (SpectrumOrTime) field in the VRT Packet Header.
 * Refer to VITA 49.2-2017 Table 5.1.1.1-1, rules 7.1.1-3, 7.1.1-4, 7.1.1-5.
 */
enum class SpectrumOrTime : bool
{
    TIME = 0b0, //!< Conveys that the vector contains spectrum data
    SPECTRUM = 0b1, //!< Conveys that the vector contains time data
}; // end enum class SpectrumOrTime
