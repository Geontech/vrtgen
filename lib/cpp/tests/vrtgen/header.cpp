...
TEST_CASE("Indicator Bits 5.1.1.1", "[header][indicator_bits]")
{
    SECTION("Data Header")
    {
        ...
        SECTION("Signal Spectrum or Signal Time Data Packet")
        {
            ...
            // Setter
            header.spectrum_or_time(vrtgen::packing::SpectrumOrTime::SPECTRUM);
            // Getter check set value
            CHECK(header.spectrum_or_time() == vrtgen::packing::SpectrumOrTime::SPECTRUM);
            // Pack
            header.pack_into(packed_bytes.data());
            // Verify packed bits
            ...
            unpack_header.unpack_from(packed_bytes.data());
            // Verify unpacked value
            CHECK(unpack_header.spectrum_or_time() == vrtgen::packing::SpectrumOrTime::SPECTRUM);
        }
    }
