...
using namespace example::packets;
...
int main()
{
    try {
        ...
        example::Controller controller({ ip, controller_cmd_port });

        // Point socket to radio endpoint
        controller.controllee_endpoint({ ip, controllee_cmd_port });
        std::cout << "Using controllee at " << controller.controllee_endpoint().to_string() << std::endl;
